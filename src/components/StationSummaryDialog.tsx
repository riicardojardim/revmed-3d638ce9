...
export function StationSummaryDialog({ specialty, title, stationId, triggerLabel = "Ver resumo", triggerClassName }: Props) {
  ...
  const sections = summary
    ? [
        ...
        { icon: Stethoscope, title: "Quadro clínico", text: summary.clinical_picture, tone: "default" as const },
        { icon: Microscope, title: "Diagnóstico", text: summary.diagnosis, tone: "default" as const },
  ...
  const sources = markerIdx >= 0
    ? raw.slice(markerIdx + marker.length).trim().split("\n").map((l) => l.replace(/^[•\-\*]\s*/, "").trim()).filter(Boolean)
    : [];
      ...
      <DialogTrigger asChild>
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[10px] font-semibold uppercase tracking-wider text-violet-300">Material de apoio</div>
            ...
            <div className="relative shrink-0 overflow-hidden bg-gradient-hero px-5 pb-6 pt-7 text-white sm:px-6 sm:pb-7">
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <DialogClose className="-mr-1 -mt-1 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-background/90 text-foreground shadow-lg ring-1 ring-white/50 backdrop-blur-md transition-all hover:bg-background focus:outline-none focus:ring-2 focus:ring-white">
                    <X className="h-4 w-4" />
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <sec.icon
                          className={cn(
                            "h-4 w-4",
                            sec.tone === "highlight" ? "text-mint" : sec.tone === "warn" ? "text-amber-500" : "text-muted-foreground",
                          )}
                        />
                        <h3 className="font-display text-sm font-bold uppercase tracking-wide">{sec.title}</h3>
                      </div>
                    </div>
                    <div className="mt-3 whitespace-pre-wrap text-[14px] leading-relaxed text-foreground/90">{sec.text}</div>
                  </section>
                ))
              ) : notes ? (
